...
export const DatetimePicker: React.FC<Props> = ({
    ...
    useEffect(() => {
        ...
        if (!isNaN(initialDate.getDate())) {
            setDatetimeString(dayjs(initialDate).format('YYYY-MM-DD HH:mm'));
        } else {
    ...
    return (
        <View style={[globalStyles.inputContainer, { width }]}>
            ...
            >
                <Text>{datetimeString || 'Select a datetime'}</Text>
            </TouchableOpacity>
            ...
            <DateTimePicker
                ...
                onConfirm={(date) => {
                    onChangeValue(dayjs(date.toString()).format('YYYY-MM-DD HH:mm'));
